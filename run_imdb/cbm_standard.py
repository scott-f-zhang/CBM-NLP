...
def get_cbm_standard(mode=None, max_len=None, batch_size=None, model_name=None, num_epochs=None, data_type=None, optimizer_lr=None):
    ...
    if model_name == 'roberta-base':
        ...
        model = BiLSTMWithDotAttention(len(tokenizer.vocab), 300, 128, num_classes=num_labels, pretrained_embeddings=embeddings)
    ...
    opt_lr = 1e-5 if optimizer_lr is None else optimizer_lr
    optimizer = torch.optim.Adam(list(model.parameters()) + list(classifier.parameters()), lr=opt_lr)
    if model_name == 'lstm':
