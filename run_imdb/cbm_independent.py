...
def get_cbm_independent(mode=None, max_len=None, batch_size=None, model_name=None, num_epochs=None, data_type=None, optimizer_lr=None):
    ...
    data_type = "gen_imdb" if data_type is None else data_type
    ...
    #step 2  CtoY
    c_to_y_epochs = 1 if num_epochs is None else num_epochs
    ...
    # Set up the optimizer and loss function
    optimizer = torch.optim.Adam(ModelCtoY_layer.parameters(), lr=1e-3 if optimizer_lr is None else optimizer_lr, weight_decay=1e-3)
    loss_fn = torch.nn.CrossEntropyLoss()
