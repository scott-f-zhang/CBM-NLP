...
def get_cbm_joint(mode=None, max_len=None, batch_size=None, model_name=None, num_epochs=None, data_type=None, optimizer_lr=None):
    ...
    data_type = "manual_imdb" if data_type is None else data_type
    # "manual_imdb"/"aug_manual_imdb"/"gen_imdb"/"aug_gen_imdb"
    # Load data
    ...
    optimizer = torch.optim.Adam(list(model.parameters()) + list(ModelXtoCtoY_layer.parameters()), lr=opt_lr)
    if model_name == 'lstm':
    ...
    ####################### test
    test_epochs = 1 if num_epochs is None else num_epochs
    print("Test!")
    model = torch.load("./"+model_name+"_joint.pth", weights_only=False)
    ModelXtoC_layer = torch.load("./"+model_name+"_ModelXtoCtoY_layer_joint.pth", weights_only=False) 
    ...
    for epoch in range(test_epochs):
        ...
        concept_acc = concept_test_accuracy*100/num_concept_labels
        concept_macro_f1 = concept_mean_macro_f1_score*100
        
        test_acc = test_accuracy*100
        test_macro_f1 = mean_macro_f1_score*100

        print(f"Epoch {epoch + 1}: Test concept Acc = {concept_acc} Test concept Macro F1 = {concept_macro_f1}")
        print(f"Epoch {epoch + 1}: Test Acc = {test_acc} Test Macro F1 = {test_macro_f1}")
        
        scores.append(concept_acc, concept_macro_f1, test_acc, test_macro_f1)
