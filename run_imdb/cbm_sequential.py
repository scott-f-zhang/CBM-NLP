...
def get_cbm_sequential(mode=None, max_len=None, batch_size=None, model_name=None, num_epochs=None, data_type=None, optimizer_lr=None):
    ...
    model_name = 'bert-base-uncased' if model_name is None else model_name # 'bert-base-uncased' / 'roberta-base' / 'gpt2' / 'lstm'
    ...
    data_type = "manual_imdb" if data_type is None else data_type # "manual_imdb"/"aug_manual_imdb"/"gen_imdb"/"aug_gen_imdb"
    # Load data
    ...
    opt_lr = optimizer_lr if optimizer_lr is not None else 1e-5
    optimizer = torch.optim.Adam(list(model.parameters()) + list(ModelXtoC_layer.parameters()), lr=opt_lr)
    if model_name == 'lstm':
    ...
    print("train XtoC!")
    x_to_c_epochs = num_epochs
    for epoch in range(x_to_c_epochs):
        ...
        print(f"Epoch {epoch + 1}: Val Concept Acc = {val_accuracy*100/num_concept_labels} Val Concept Macro F1 = {mean_macro_f1_score*100}")
        if val_accuracy > best_acc_score:
            best_acc_score = val_accuracy
            best_predicted_concepts = predict_concepts
            best_labels = labelY
            torch.save(model, "./"+model_name+"_sequential.pth")
            torch.save(ModelXtoC_layer, "./"+model_name+"_ModelXtoC_layer_sequential.pth")

    #step 2  CtoY
    c_to_y_epochs = num_epochs
    ...
    # Set up the optimizer and loss function
    optimizer = torch.optim.Adam(ModelCtoY_layer.parameters(), lr=1e-3 if optimizer_lr is None else optimizer_lr)
    loss_fn = torch.nn.CrossEntropyLoss()
    ...
    for epoch in range(c_to_y_epochs):
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc="Val", unit="batch"):
                ...
                CtoY_logits = ModelCtoY_layer(concept_labels)
        ...
        if val_accuracy > best_acc_score:
            best_acc_score = val_accuracy
            torch.save(ModelCtoY_layer, "./"+model_name+"_ModelCtoY_layer_sequential.pth")
