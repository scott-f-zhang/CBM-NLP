#!/usr/bin/env python3
"""BERT-only test runner for PLMs, CBE-PLMs, and CBE-PLMs-CM with switches."""
import os
import sys
import argparse
import pandas as pd
...
OUTPUT_CSV = os.path.join(TESTS_DIR, "test_results", "result_bert_test.csv")


def get_learning_rate(model_name: str) -> Optional[float]:
    return {"bert-base-uncased": 1e-5}.get(model_name)
...
def run_experiments_for_function(func_name: str, func, datasets_sel, variants_filter: str, metrics_filter: str):
    rows = []
    print(f"Running {func_name} (BERT)...")
...
def build_pivot_table(df: pd.DataFrame):
    ...
    if not df_cebab.empty:
        ...
        dfp_c['model'] = dfp_c['model'].map({'bert-base-uncased': 'BERT'}).fillna(dfp_c['model'])
    ...
    if not df_imdb.empty:
        ...
        dfp_i['model'] = dfp_i['model'].map({'bert-base-uncased': 'BERT'}).fillna(dfp_i['model'])
...
def main():
    parser = argparse.ArgumentParser(description="BERT test with dataset/variant/metric filters")
