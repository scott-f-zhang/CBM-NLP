#!/usr/bin/env python3
"""GPT2-only test runner for PLMs, CBE-PLMs, and CBE-PLMs-CM with switches.

Usage examples:
- python main/test_gpt2.py --datasets cebab --variants D^
- python main/test_gpt2.py --datasets all --variants all --metrics task
"""
import os
import sys
import argparse
...
OUTPUT_CSV = os.path.join(TESTS_DIR, "test_results", "result_gpt2_test.csv")
...
def run_experiments_for_function(func_name: str, func, datasets_sel, variants_filter: str, metrics_filter: str):
    rows = []
    print(f"Running {func_name} (GPT2)...")
...
def build_pivot_table(df: pd.DataFrame):
    ...
    if not df_cebab.empty:
        ...
        dfp_c['model'] = dfp_c['model'].map({'gpt2': 'GPT2'}).fillna(dfp_c['model'])
    ...
    if not df_imdb.empty:
        ...
        dfp_i['model'] = dfp_i['model'].map({'gpt2': 'GPT2'}).fillna(dfp_i['model'])
...
def main():
    parser = argparse.ArgumentParser(description="GPT2 test with dataset/variant/metric filters")
